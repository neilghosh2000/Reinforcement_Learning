...
class vishamC(Env):
    ...
    def _step(self, action):
        ...
        gamma = 10
        self.state = np.array([self.state[0] + action[0], self.state[1] + action[1]])       # (action[0],action[1]) is (dx,dy)

        reward = -0.5 * (np.square(self.state[0]) + gamma * np.square(self.state[1]))

        if reward > -0.10:               # Stop if current state is very close to the target
