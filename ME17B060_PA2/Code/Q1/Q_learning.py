...
class QLearning:
    ...
    def __init__(self, env, episodes, target, gamma=0.9, alpha=0.15, epsilon=0.1):
    ...
    def action(self, curr_state):
        if np.random.binomial(1, self.epsilion) == 1:
            return np.random.randint(0, self.env.action_space.n)

        max_q = max(self.Q[curr_state[0], curr_state[1], :])
        indices = [index for index, val in enumerate(self.Q[curr_state[0], curr_state[1], :]) if val == max_q]
    ...
    def run(self):
        ...
        for i in range(self.episodes):
            ...
            while True:
                ...
                # TD Update
                target = reward + self.gamma * max(self.Q[next_state[0], next_state[1], :])
    ...
    def show_policy(self):
        ...
        cmap = colors.ListedColormap([(0, 0, 0), (0.3, 0.3, 0.3),  (0.6, 0.6, 0.6), (0.9, 0.9, 0.9), (1, 0, 0), (0, 0, 0.8)])
