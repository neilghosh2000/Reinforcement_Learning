...
class SARSA_Lambda:
    ...
    def __init__(self,  env, episodes, target, lam, gamma=0.9, alpha=0.1):
        self.Q = np.zeros((env.height, env.width, env.action_space.n))
        self.e = np.zeros((env.height, env.width, env.action_space.n))
    ...
    def action(self, curr_state):       # Softmax action selection
    ...
    def run(self):
        ...
        self.e = np.zeros((self.env.height, self.env.width, self.env.action_space.n))
        ...
        for i in range(self.episodes):
            ...
            while True:
                ...
                self.e[curr_state[0], curr_state[1], next_action] += 1
